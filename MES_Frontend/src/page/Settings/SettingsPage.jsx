...
export default function SettingsPage() {
  ...
  const computeNextId = (empList) => {
    const nums = empList
      .map(e => parseInt((e.employee_no || '').replace('BR-', ''), 10))
  ...
  const fetchEmployees = () => {
    apiFetch('/employees/').then(r => r.json()).then(data => {
      const list = Array.isArray(data) ? data : [];
      setEmployees(list);
      setNextEmpId(computeNextId(list));
    });
  };
 ...
 const handleAdd = async () => {
    if (!newEmployee.last_name.trim()) {
      modal.error({ title: 'Validation Error', content: 'Employee name is required.' });
      return;
    }
    if (!/^[a-zA-Z\s]+$/.test(newEmployee.last_name)) {
      modal.error({ title: 'Validation Error', content: 'Employee name cannot contain special characters or numbers.' });
      return;
    }
    if (!newEmployee.role_ids.length) {
      modal.error({ title: 'Validation Error', content: 'Please select at least one role.' });
      return;
    }
    if (!newEmployee.password) {
      modal.error({ title: 'Validation Error', content: 'Password is required.' });
      return;
    }
    const empNo = `BR-${(newEmployee.employeeId || nextEmpId).padStart(8, '0')}`;
    if (!/^\d{1,8}$/.test(newEmployee.employeeId || nextEmpId)) {
      modal.error({ title: 'Validation Error', content: 'Employee ID must be numeric.' });
      return;
    }
    ...
    if (response.ok) {
      ...
      modal.success({ title: 'Employee Added', content: `Employee '${newEmployee.last_name}' (Employee ID: ${empNo}) has been added successfully.` });
    } else {
...
const handleToggleStatus = (record) => {
  ...
  const handleEditSave = async () => {
    ...
    if (response.ok) {
      ...
      modal.success({ title: 'Employee Updated', content: 'Employee record updated successfully.' });
    }
  };
  ...
  const handleAddRole = async () => {
    ...
    if (response.ok) {
      ...
      modal.success({ title: 'Role Added', content: `Role '${newRole.role_name}' has been created successfully with access to: ${newRole.pages.map(p => PAGE_OPTIONS.find(o => o.value === p)?.label || p).join(', ')}.` });
    } else {
  ...
  const handleEditRoleSave = async () => {
    ...
    if (response.ok) {
      ...
      setEditRole(null);
      modal.success({ title: 'Role Updated', content: 'Role updated successfully.' });
    } else {
      const err = await response.json();
  ...
  const handleDeleteRole = (record) => {
    modal.confirm({
      ...
      onOk: async () => {
        ...
        if (response.ok) {
          fetchRoles();
          modal.success({ title: 'Role Deleted', content: `Role '${record.role_name}' has been permanently deleted.` });
        } else {
          const err = await response.json();
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: constants.spacing.md, marginBottom: constants.spacing.md, flexWrap: 'wrap' }}>
            <Input
              ...
              prefix="BR-"
              value={newEmployee.employeeId !== '' ? newEmployee.employeeId : nextEmpId}
              onChange={(e) => setNewEmployee({ ...newEmployee, employeeId: e.target.value.replace(/[^0-9]/g, '') })}
              style={{ width: '180px' }}
              maxLength={8}
              autoComplete="off"
            />
          ...
          <Table
            ...
            dataSource={[...employees].sort((a, b) => b.id - a.id).filter(emp=>
